...
timeout "${RUN_TIMEOUT}" make run_test \
  ...
  TESTCASE="${TESTCASE}" \
  "${make_args[@]}" \
  RUN_PLUSARGS_EXTRA="+RST_RELEASE=${RST_RELEASE}${DTCMCASE:+ +DTCMCASE=${DTCMCASE}} ${EXTRA_PLUSARGS}"
run_rc=$?
set -e

test_name="$(basename "${TESTCASE}")"
latest_log="$(find "${VSIM_DIR}/run" -maxdepth 2 -name "${test_name}.log" | head -n 1 || true)"
